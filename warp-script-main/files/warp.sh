...
check_tun() {
    ...
    if [[ ! $TUN =~ "in bad state"|"处于错误状态"|"ist in schlechter Verfassung" ]]; then
        if [[ $VIRT == lxc ]]; then
            ...
            wget -N --no-check-certificate https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/tun.sh && bash tun.sh
        else
            red "检测到目前VPS未开启TUN模块, 请到后台控制面板处开启"
            exit 1
        fi
...
check_endpoint() {
    ...
    wget https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/warp-yxip/warp-linux-$(archAffix) -O warp >/dev/null 2>&1
...
# 下载 WGCF
init_wgcf() {
    wget --no-check-certificate https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/wgcf/wgcf-latest-linux-$(archAffix) -O /usr/local/bin/wgcf
...
register_wgcf() {
    if [[ $country4 == "Russia" || $country6 == "Russia" ]]; then
        # 下载 WARP API 工具
        wget https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/warp-api/main-linux-$(archAffix)
...
install_wgcf() {
    ...
    if [[ $SYSTEM == "CentOS" ]]; then
        ...
        if [[ $OSID == 9 ]] && [[ -z $(type -P resolvconf) ]]; then
            wget -N https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/resolvconf -O /usr/sbin/resolvconf
    ...
    if [[ $main -lt 5 ]] || [[ $minor -lt 6 ]] || [[ $VIRT =~ lxc|openvz ]]; then
        wget -N --no-check-certificate https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/wireguard-go/wireguard-go-$(archAffix) -O /usr/bin/wireguard-go
...
install_wpgo() {
    ...
    # 下载 WARP-GO
    mkdir -p /opt/warp-go/
    wget -O /opt/warp-go/warp-go https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/warp-go/warp-go-latest-linux-$(archAffix)
...
install_wireproxy() {
    ...
    # 下载 WireProxy
    wget -N https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/wireproxy/wireproxy-latest-linux-$(archAffix) -O /usr/local/bin/wireproxy
...
warp_traffic() {
    ...
    wget -N --no-check-certificate https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/wp-plus.py
...
before_showinfo() {
    ...
    if [[ ! -f /usr/local/bin/nf ]]; then
        wget https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/netflix-verify/nf-linux-$(archAffix) -O /usr/local/bin/nf >/dev/null 2>&1
...
menu() {
    ...
    case $menu_input in
        ...
        12) warp_traffic ;;
        13) warp_account ;;
        14) wget -N https://ghproxy.net/https://ghproxy.net/https://raw.githubusercontent.com/xiaolinshao/warp-IP/main/warp-script-main/files/warp.sh && bash warp.sh ;;
        *) exit 1 ;;
    esac
}
